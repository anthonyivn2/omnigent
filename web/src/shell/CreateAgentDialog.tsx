 ...
 *
 * Three sources (tabs): a field Form, a File upload (single YAML or a
 * pre-built `.tar.gz`), and a Folder upload (a whole bundle directory). Each
 * produces a {@link PendingAgentSource} passed back via `onCreate`; the parent
 * builds/uploads the bundle and starts a session. Browser uploads keep
 * `${VAR}` references literal (no env), so secrets must never be committed to
 * an uploaded bundle.
 */
export function CreateAgentDialog({
  ...
  const [tab, setTab] = useState<"form" | "file" | "folder">("form");
