...
export function NewChatLandingScreen() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (effectiveAgentId === PENDING_AGENT_ID && pendingAgent) {
        // Custom agent path: use multipart POST. The multipart create only
        // stores the agent + session rows — it does NOT launch a runner on the
        // host. We must follow up with launchRunner (POST /v1/hosts/{id}/runners)
        // to bind the session to a runner, the same way the fork-resume path does.
        // "form" builds the bundle client-side; "bundle" (folder / file /
        // tarball) is already a built File.
        const bundle =
